...
%%*************************************************************************
function [u,w,Atxi,xi,par,runhist,info,Ainput] = ...
    ppa_sub_logistic(b,Ainput,x0,Ax0,Atxi0,xi0,lambda1,group_info,par,options)
iftest = 0;
printsub = 1;
...
par.lsAmap = Amap;
par.group_info = group_info;
par.lambda1 = lambda1;
par.b = b;

uinput = x0 - sig*Atxi0;
[up,info_u] = prox_exclusive(uinput, n, sig*lambda1, group_info); 
par.info_u = info_u;
u = (uinput -up)/sig;
...
Ly = Ly + lambda1*xgroupnorm(up,group_info) + sum(log(1+exp(-b.*wp)));
runhist.psqmr(1) = 0;
...
for itersub = 1:maxitersub
    ...
    psix = sum(log(1+exp(-b.*Aup))) + lambda1*xgroupnorm(up,group_info) + norm(up-x0)^2/(2*sig) + norm(Aup-Ax0)^2/(2*sigdtau);
%     tolsub = 1e-8;
    ...
    [dxi,resnrm,solve_ok,par,Ainput] = ppa_logistic_exclusive_solve_xi(Ainput,rhs,par);
    ...
    [par,Ly,xi,Atxi,u,up,w,wp,alp,iterstep] = ...
        findstep(par,n,b,lambda1,group_info,Ly,xi,Atxi,...
...
function [par,Ly,xi,Atxi,u,up,w,wp,alp,iter] = ...
    findstep(par,n,b,lambda1,group_info,Ly0,xi0,Atxi0,...
