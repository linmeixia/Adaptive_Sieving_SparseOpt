...
%%*************************************************************************
function [u,Atxi,xi,par,runhist,info,Ainput] = ...
    NCGExclusivelasso(b,Ainput,x0,Atxi0,xi0,lambda1,group_info,par,options)
iftest = 0;
printsub = 1;
...
Atxi = Atxi0; xi = xi0;
Ly = -b'*xi - 0.5*norm(xi)^2  - norm(up)^2/(2*sig) - pstar(u,lambda1,group_info);
runhist.psqmr(1) = 0;
...
for itersub = 1:maxitersub
    ...
    if max(normGradLxi) < max(tolsub) && itersub > 1
        ...
        if printsub
            fprintf('\n       %s  ',msg);
            fprintf(' normRd=%3.2e, gradLxi = %3.2e, tolsub=%3.2e',...
    ...
    [dxi,resnrm,solve_ok,par,Ainput] = LS_exclusive_solve_xi(Ainput,rhs,par);
