function [u,Atxi,xi,par,runhist,info] = NCGWK(b,Ainput,x0,Ax0,Atxi0,xi0,...
    lambda1,par,options)
...
par.lsAmap  = Amap;
uinput      = x0 - sig*Atxi0;
[up,info_u] = proxSortedL1(uinput,sig*lambda1); 
par.info_u  = info_u;
u           = (uinput -up)/sig;
Rp          = Ax0 - b - xi0;
normRp      = norm(Rp);
...
for itersub = 1:maxitersub    
    ...
    %% Compute Newton direction
     ...
     [dxi,resnrm,solve_ok,par] = WKsolve(Ainput,rhs,par); 
     Atdxi                     = ATmap(dxi);
     iterpsqmr                 = length(resnrm)-1;
     if (printsub)
          fprintf('| %3.1e %3.1e %3.0d',par.tol,resnrm(end),iterpsqmr);
          fprintf(' %2.1f [%2d, %2d, (%2d, %2d)]',const2,sum(par.info_u.rr2),sum(par.info_u.nz), par.lenP, par.numblk1);
     end
     par.iter = itersub;
     ...
     [par,Ly,xi,Atxi,u,up,alp,iterstep] = findstep(par,b,lambda1,Ly,xi,Atxi,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [par,Ly,xi,Atxi,u,up,alp,iter] = findstep(par,b,lambda1,Ly0,...
         xi0,Atxi0,u0,up0,dxi,Atdxi,tol,options)
