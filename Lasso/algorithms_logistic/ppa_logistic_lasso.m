function [obj,x,xi,u,info,runhist] = ppa_logistic_lasso(Ainput,b,n,lambda1,options,x0,xi0,u0)
rng('default');

iftest = 0;
maxiter = 5000; % maximum iteration of the SSNAL
...
if admm.op.maxiter > 0 && runphaseI
    ...
    [obj,xi,u,x,info_admm,runhist_admm] = startfun(Ainput,borg,n,lambda1org,admm.op,x,xi,u);
    ...
    obj(1) = sum(tmp) + lambda1org*norm(x,1);
...
if printyes
    fprintf('\n *******************************************************');
    fprintf('******************************************');
    fprintf('\n \t\t   Phase II: PPDNA for solving Lasso problem ');
    fprintf('\n ******************************************************');
    fprintf('*******************************************\n');
...
if SSNCG
    ...
    for iter = 1:maxiter
        ...
        [u,w,Atxi,xi,parNCG,~,info_NCG] = ...
            ppa_sub_logistic_lasso(b,Ainput_nal,x,Ax,Atxi,xi,lambda1,parNCG,ssncgop);
        ...
        %%---------------------------------------------------------
        tmp = log(1+exp(-b.*Ax));
        primobj = objscale*(sum(tmp) + lambda1*norm(x,1));
        tmp = (1+b.*xi).*log(1+b.*xi)+(-b.*xi).*log(-b.*xi);
        dualobj = objscale*(-sum(tmp));
        ...
        if stopop == 1
            if (max([primfeasorg,dualfeasorg]) < 500*max(1e-6, stoptol))
                ...
                etaorg = errcom(x*bscale,grad,lambda1org);
...
tmp = log(1+exp(-borg.*Ax));
primobj = sum(tmp) + lambda1org*norm(x,1);
tmp = (1+borg.*xi).*log(1+borg.*xi)+(-borg.*xi).*log(-borg.*xi);
dualobj = -(sum(tmp));
relgap = (primobj-dualobj)/( 1+abs(primobj)+abs(dualobj));
obj = [primobj, dualobj];
grad = ATmap0(Rp1);
etaorg = errcom(x,grad,lambda1org);
...
function etaorg = errcom(x,grad,lambda1)
tmp = x - grad;
tmp2 = proxL1(tmp, lambda1);
etaorg = norm(x - tmp2);
end
